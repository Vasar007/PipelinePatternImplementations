...
{
    internal sealed class TPLPipelineWithAwaitFinal<TIn, TOut>
    {
        private readonly List<IDataflowBlock> _steps = new List<IDataflowBlock>();

        public TPLPipelineWithAwaitFinal()
        {
        }

        public void AddStep<TLocalIn, TLocalOut>(Func<TLocalIn, TLocalOut> stepFunc)
        {
            var step = new TransformBlock<TC<TLocalIn, TOut>, TC<TLocalOut, TOut>>(
                tc => new TC<TLocalOut,TOut>(stepFunc(tc.Input), tc.TaskCompletionSource)
            );
        ...
        {
            var firstStep = _steps.First() as ITargetBlock<TC<TIn, TOut>>;
